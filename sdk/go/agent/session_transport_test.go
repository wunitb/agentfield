...
func TestValidateSessionTransportRejectsProviderTransportMismatch(t *testing.T) {
	...
	if got := err.Error(); got == "" || !containsAllSessionTransport(got, []string{"audio_turns", "does not infer or switch providers"}) {
...
func TestValidateSessionTransportRequiresExplicitTransport(t *testing.T) {
	...
	if err == nil || !containsAllSessionTransport(err.Error(), []string{"transport is required"}) {
...
func TestValidateSessionTransportRejectsUnknownProvider(t *testing.T) {
	...
	if err == nil || !containsAllSessionTransport(err.Error(), []string{"unknown session provider", "custom"}) {
		t.Fatalf("unexpected error: %v", err)
	}
}

func containsAllSessionTransport(value string, needles []string) bool {
