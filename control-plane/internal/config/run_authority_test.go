...
func TestRunAuthorityConfigDefaultsAndEnvironmentOverrides(t *testing.T) {
	...
	t.Setenv("AGENTFIELD_RUN_AUTHORITY_EXPECTED_RUNNER_TYPE", "agentfield")
	...
	if cfg.AgentField.RunAuthority.ExpectedRunnerType != "agentfield" {
		t.Fatalf("unexpected runner type default: %q", cfg.AgentField.RunAuthority.ExpectedRunnerType)
	}

	ApplyEnvOverrides(&cfg)
	authority := cfg.AgentField.RunAuthority
	if !authority.Enabled || authority.BaseURL != "https://deputies.example.test" || authority.ExpectedHomeID != "home-a" || authority.ExpectedRunnerType != "agentfield" {
