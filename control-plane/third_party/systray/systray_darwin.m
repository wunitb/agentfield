...
// AgentField patch: set a non-template menu-item image at an explicit point
// size. This mirrors setMenuItemIcon but (1) sizes the NSImage to the caller's
// point dimensions instead of the hard-coded 16x16 clamp, and (2) never marks
// the image as a template, so wide colored charts and bars render at full width
// and in their own colors. It reuses the existing setMenuItemIcon: selector,
// which simply assigns menuItem.image. See PATCHES.md.
...
// size, non-template. This mirrors setIcon but (1) sizes the NSImage to the
// caller's point dimensions instead of the hard-coded 16x16 clamp, and (2) never
// marks the image as a template, so a wide colored menu-bar widget renders at
// full width and in its own colors alongside the (native) title. It reuses the
// existing setIcon: selector, which assigns statusItem.button.image and updates
// the image/title layout. Calling setIcon afterward restores the plain icon for
// the fallback state. See PATCHES.md.
